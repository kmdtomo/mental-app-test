...
export function SignupForm({ className, ...props }: UserSignupFormProps) {
  const { toast } = useToast();

  const [state, formAction] = useFormState(
    async (prevState: SignupFormActionResult, formData: FormData) => {
      const result = await signupWithEmailFormAction(prevState, formData);
      if (result.success) {
        toast({
          title: "成功",
...
type FormContentProps = {
  formState: SignupFormActionResult;
};
      ...
      <div className="grid gap-1">
        ...
        <div className="space-y-1">
          <Input
            ...
            autoCorrect="off"
          />
          {formState.errors?.fieldErrors?.["confirm_password"] && (
            <p className="text-xs text-red-500">
              {formState.errors.fieldErrors["confirm_password"][0]}
            </p>
          )}
        </div>
